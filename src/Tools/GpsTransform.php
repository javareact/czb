...
/**
 * Class GpsTransform
 * @example WGS84坐标系：即地球坐标系，国际上通用的坐标系。<br>
 *  GCJ02坐标系：即火星坐标系，WGS84坐标系经加密后的坐标系。Google Maps，高德在用。
 *  BD09坐标系：即百度坐标系，GCJ02坐标系经加密后的坐标系。
 *  lat 纬度 log 经度
 * @package JavaReact\CzbApi\Tools
 */
class GpsTransform
{
     ...
     * @return array
     */
    private function delta(float $lon, float $lat): array
    {
     ...
     * @return array
     */
    private function transform(float $lon, float $lat): array
    {
        $lonlat = $lon * $lat;
        $absX   = sqrt(abs($lon));
        $lonPi  = $lon * M_PI;
     ...
     * @return bool
     */
    private function isOutOFChina(float $lon, float $lat): bool
    {
